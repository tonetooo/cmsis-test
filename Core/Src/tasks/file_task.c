...
void StartFileTask(void *argument) {
    printf("[FILE] Task started (prio=Normal)\r\n");
    printf("[FILE] Waiting for sensor data on queue...\r\n");
    ...
    for (;;) {
        ...
        if (status == osOK) {
            ...
            if (osMutexAcquire(sd_mutexHandle, 10) != osOK) {
                ...
                printf("[FILE][MUTEX] Wait #%lu (contention #%lu)... waiting indefinitely\r\n",
            ...
            if (!file_open) {
                ...
                printf("[FILE] Scanning for next available TRIG_XXX.CSV...\r\n");
                for (int i = 1; i < 999; i++) {
                    sprintf(filename, "TRIG_%03d.CSV", i);
                    if (f_open(&fil, filename, FA_READ) != FR_OK) break;
                    f_close(&fil);
                }
                printf("[FILE] Next filename: %s\r\n", filename);
                ...
                if (f_open(&fil, filename, FA_CREATE_ALWAYS | FA_WRITE) == FR_OK) {
                    ...
                    printf("[FILE][CSV] Header written (%u bytes): \"%s\"\r\n", bw, header);
                    f_sync(&fil);
                    file_open = 1;
                    write_count = 0;
                    printf("[FILE] [OK] Opened %s for writing\r\n", filename);
                } else {
                    printf("[FILE] [FAIL] Could not create %s\r\n", filename);
                }
            }
            ...
            if (file_open) {
                ...
                sprintf(buf, "%lu.%03lu;%lu.%03lu;%lu.%03lu;%.6f;%.6f;%.6f;%.2f;%.2f;%.2f\r\n",
                        rel_sec, rel_us / 1000,
                        abs_sec, rel_us / 1000,
                        abs_sec, rel_us / 1000,
                ...
                write_count++;
                if (write_count <= 3) {
                    printf("[FILE][CSV] Sample #%lu: %s", (unsigned long)write_count, buf);
                }
